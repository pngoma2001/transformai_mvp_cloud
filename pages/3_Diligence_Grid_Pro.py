...
def ensure_state():
    ...
    # What-if inputs used by Unit Economics (kept for compatibility)
...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
# REPLACED: Real cohort math (only change requested)
def _cohort(df: pd.DataFrame) -> Dict[str, Any]:
    """
    Real cohort math

    Cohort = customer's first purchase month (YYYY-MM).
    Retention at month k = % of that cohort with a purchase at cohort_month + k.
    - Ignores tiny cohorts (min_cohort_size) to reduce noise
    - Caps horizon at max_horizon months
    Returns:
      - value: M3 retention (average across eligible cohorts)
      - curve: average retention curve (list of floats)
      - heatmap: 2D list [cohort x month_offset] of retention (0..1)
      - cohort_labels: list of cohort month strings aligned with heatmap rows
      - cohort_sizes: list of (cohort, size) for all cohorts
      - evidence: list of {cohort, m0, m3_cnt, m3_ret} for memo proof
    ...
    try:
        # --- 1) Normalize inputs we need: customer_id and a monthly period ---
        need_cols = {"customer_id"}
        if not need_cols.issubset(set(df.columns)):
            raise ValueError("customer_id column missing")

        d = df[["customer_id"] + [c for c in ["order_date", "month"] if c in df.columns]].copy()

        if "order_date" in d.columns:
            d["order_month"] = pd.to_datetime(d["order_date"], errors="coerce").dt.to_period("M")
        elif "month" in d.columns:
            d["order_month"] = pd.PeriodIndex(d["month"].astype(str), freq="M")
        else:
            raise ValueError("Need order_date or month for cohort math")

        d = d.dropna(subset=["customer_id", "order_month"])
        if d.empty:
            raise ValueError("No valid rows after parsing dates")

        # --- 2) Cohort = first purchase month per customer ---
        first = (
            d.groupby("customer_id", observed=True)["order_month"]
            .min()
            .rename("cohort")
        )
        d = d.join(first, on="customer_id", how="left")
        d["offset"] = (d["order_month"] - d["cohort"]).astype(int)
        d = d[d["offset"] >= 0]

        # --- 3) Base sizes and activity by (cohort, offset) ---
        base_sizes = (
            d.groupby("cohort", observed=True)["customer_id"]
            .nunique()
            .astype(int)
        )  # m0 cohort sizes (unique customers)
        activity = (
            d.groupby(["cohort", "offset"], observed=True)["customer_id"]
            .nunique()
            .astype(int)
        )

        if base_sizes.empty:
            raise ValueError("No cohorts could be formed")

        # --- 4) Horizon and cohort selection (filter tiny cohorts) ---
        max_seen_offset = int(activity.index.get_level_values("offset").max()) if not activity.empty else 0
        horizon = max(1, min(MAX_H, max_seen_offset + 1))

        cohorts_sorted = sorted(base_sizes.index.tolist())
        cohort_labels_all = [str(c) for c in cohorts_sorted]
        cohort_sizes_list = [(str(c), int(base_sizes.get(c, 0))) for c in cohorts_sorted]

        eligible_cohorts = [c for c in cohorts_sorted if base_sizes.get(c, 0) >= MIN_SIZE]
        if not eligible_cohorts:
            eligible_cohorts = [c for c, _ in sorted(base_sizes.items(), key=lambda x: -x[1])[:3]]

        # --- 5) Build heatmap rows (retention per cohort x month_offset) ---
        heatmap_rows: List[List[Optional[float]]] = []
        heatmap_labels: List[str] = []
        for c in eligible_cohorts:
            base = float(base_sizes.get(c, 0))
            row = []
            for k in range(horizon):
                cnt = float(activity.get((c, k), 0.0))
                row.append(cnt / base if base > 0 else np.nan)
            heatmap_rows.append(row)
            heatmap_labels.append(str(c))

        # --- 6) Average curve across cohorts (ignore NaNs) ---
        if heatmap_rows:
            arr = np.array(heatmap_rows, dtype=float)
            curve = np.nanmean(arr, axis=0)
            curve = np.where(np.isnan(curve), 0.0, curve).tolist()
        else:
            curve = []

        m3 = curve[3] if len(curve) > 4 else (curve[3] if len(curve) > 3 else None)

        # --- 7) Evidence table (m0, m3) ---
        evidence = []
        for c in cohorts_sorted:
            m0 = int(base_sizes.get(c, 0))
            m3_cnt = int(activity.get((c, 3), 0)) if horizon > 3 else 0
            m3_ret = (m3_cnt / m0) if m0 > 0 else None
            evidence.append(
                dict(cohort=str(c), m0=m0, m3_cnt=m3_cnt, m3_ret=round(m3_ret, 4) if m3_ret is not None else None)
            )

        eligible_n = len(heatmap_rows)
        summary = (
            f"Average retention across {eligible_n} cohort(s) "
            f"(min size ≥ {MIN_SIZE}, horizon ≤ {horizon}): "
            + (f"M3 ≈ {m3:.0%}." if isinstance(m3, (int, float)) else "M3 not available.")
        )

        return dict(
            value=m3,
            curve=curve,
            heatmap=heatmap_rows,          # real cohort heatmap (row=cohort, col=month_offset)
            cohort_labels=heatmap_labels,  # labels for heatmap rows
        ...
        # Conservative fallback (keeps the rest of the UI working)
        curve = [1.0, 0.9, 0.7, 0.6, 0.55, 0.5][:12]
        m3 = curve[3] if len(curve) > 3 else None
        return dict(
            value=m3,
            curve=curve,
            heatmap=[curve],                # simple placeholder
            cohort_labels=["demo"],
            cohort_sizes=[("demo", 100)],
            evidence=[dict(cohort="demo", m0=100, m3_cnt=int(100 * (m3 or 0)), m3_ret=m3)],
            summary="Demo retention (fallback)."
        )
# <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
def execute_cell(row: Dict[str,Any], col: Dict[str,Any]) -> Dict[str,Any]:
    ...
    if mod == "Cohort Retention (CSV)":
        ...
        # keep curve; pass through new fields (heatmap, labels, evidence) harmlessly
        if "curve" in k:    out["curve"]    = k["curve"]
        if "heatmap" in k:  out["heatmap"]  = k["heatmap"]
        if "cohort_labels" in k: out["cohort_labels"] = k["cohort_labels"]
        if "cohort_sizes" in k:  out["cohort_sizes"]  = k["cohort_sizes"]
...
def plot_retention_heatmap(curve: List[float], heatmap: Optional[List[List[float]]] = None, labels: Optional[List[str]] = None):
    """
    If a real heatmap is provided (rows=cohorts, cols=month_offset), use it.
    Otherwise fallback to a simple tiled curve (same as before).
    """
    if heatmap and len(heatmap) and len(heatmap[0]):
        z = np.array(heatmap, dtype=float)
    else:
        # previous behavior (synthetic), kept intact
        curve = [float(x) for x in (curve or [])]
        if not curve:
...
with tab_review:
    ...
    if not (rid and cid):
        ...
        if not res:
            st.warning("No result yet. Click **Run** above.")
        else:
