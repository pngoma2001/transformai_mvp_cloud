...
/* Fix H1 clipping on some screens */
h1, .stMarkdown h1 {
...
# Cohort Retention included in template so retention charts appear
QOE_TEMPLATE = [
...
def run_queued_jobs():
    # Synchronous execution (no threads) to keep ScriptRunContext valid
    by_r = {r["id"]: r for r in SS["rows"]}
    by_c = {c["id"]: c for c in SS["columns"]}
    for j in SS["jobs"]:
...
with tab_data:
    ...
    c1, c2 = st.columns(2)
    with c1:
        csvs = st.file_uploader("Upload CSVs", type=["csv"], accept_multiple_files=True, key="csv_uploader")
        if csvs:
            for f in csvs:
                try:
                    df = pd.read_csv(f)
    ...
    with c2:
        pdfs = st.file_uploader("Upload PDFs", type=["pdf"], accept_multiple_files=True, key="pdf_uploader")
        if pdfs:
            for f in pdfs:
...
with tab_grid:
    ...
    with a1:
        if st.button("Add rows from CSVs", use_container_width=True, key="btn_add_rows_csv"):
            add_rows_from_csvs(); st.toast("CSV rows added")
    with a2:
        if st.button("Add rows from PDFs", use_container_width=True, key="btn_add_rows_pdf"):
    ...
    with nc1:
        new_label = st.text_input("New column label", value=SS.get("new_col_label","NRR/GRR"), key="new_col_label")
...
with tab_run:
    ...
    st.toggle("Force re-run (ignore cache)", key="force_rerun_toggle", value=SS.get("force_rerun", False))
    ...
    with st.expander("Manual run by Matrix selection", expanded=False):
        ...
        if options:
            choice = st.selectbox("Pick a row/module to run", options, format_func=lambda t: t[2], key="sel_run_one")
            if st.button("Run selected", key="btn_run_selected"):
    ...
    if SS["results"]:
        ...
        with c1:
            st.download_button(
                ...
                key="dl_results_csv_run",
            )
        with c2:
            if REPORTLAB_OK:
                st.download_button(
                    "Export memo PDF (demo)",
                    data=export_results_pdf(),
                    file_name="TransformAI_Memo_demo.pdf",
                    key="dl_memo_pdf_run",
                )
            else:
...
with tab_review:
    ...
    with csel1:
        rid = st.selectbox("Row", row_opt, format_func=lambda t: t[1], key="review_row") if row_opt else None
    with csel2:
        cid = st.selectbox("Column", col_opt, format_func=lambda t: t[1], key="review_col") if col_opt else None
    with csel3:
        if rid and cid and st.button("Retry", key="btn_retry_cell"):
    ...
    if not (rid and cid):
        ...
        if st.button("Run this cell now", type="primary", key="btn_run_cell_now"):
...
with tab_memo:
    ...
    # very simple placeholder markdown memo
    md = "# TransformAI — Investor Memo (Demo)\n\n"
    md += "- This is a placeholder memo that would be composed from approved cells.\n"
    md += "- Use the Run tab to export a PDF summary.\n"

    st.download_button(
        "Download Memo (Markdown)",
        data=md.encode("utf-8"),
        file_name="TransformAI_Memo_demo.md",
        key="dl_memo_md_tab",
    )
