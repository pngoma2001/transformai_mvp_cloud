...
# New in this patch: Segment Filters per CSV row, Approvals + Notes, Run All Mapped,
# Sheet → Run selected pair, Approved-only memo export.
...
def ensure_state():
    ...
    # Review focus
    SS.setdefault("last_focus", None)         # (rid, cid)
    SS.setdefault("pending_focus", None)      # sheet → review

    # NEW: per-CSV segment filters, keyed by CSV file name
    # SS["row_filters"][csv_name] = {"column": "region", "values": ["EMEA","NA"]}
    SS.setdefault("row_filters", {})

    # NEW: approvals + notes, keyed by (rid, cid)
    SS.setdefault("approvals", {})            # {(rid,cid): True|False}
    SS.setdefault("notes", {})                # {(rid,cid): "text"}
...
def snapshot_push():
    SS["undo"].append(json.dumps({
        ...
        "row_filters": SS["row_filters"],
        "approvals": {f"{k[0]}|{k[1]}": v for k, v in SS["approvals"].items()},
        "notes": {f"{k[0]}|{k[1]}": v for k, v in SS["notes"].items()},
    }, default=str))
    SS["redo"].clear()
...
def snapshot_apply(snap: str):
    ...
    SS["row_filters"] = data.get("row_filters", {})
    aps = data.get("approvals", {})
    nts = data.get("notes", {})
    SS["approvals"] = {tuple(k.split("|",1)): v for k, v in aps.items()} if aps else {}
    SS["notes"]     = {tuple(k.split("|",1)): v for k, v in nts.items()} if nts else {}
...
def materialize_df(csv_name: str) -> pd.DataFrame:
    ...
    # --- NEW: apply segment filter BEFORE renaming ---
    filt = SS["row_filters"].get(csv_name, {})
    fcol = filt.get("column")
    fvals = filt.get("values", [])
    if fcol and fcol in df.columns and fvals:
        try:
            mask = df[fcol].astype(str).isin([str(v) for v in fvals])
            df = df[mask]
        except Exception:
            pass
...
def cache_key_for(row: Dict[str,Any], col: Dict[str,Any]) -> str:
    ...
    # include active filter in cache key
    filt = SS["row_filters"].get(row["source"], {})
    return f"csv::{row['source']}::{col['module']}::{json.dumps(sch, sort_keys=True)}::{json.dumps(filt, sort_keys=True)}"
...
def execute_cell(row: Dict[str,Any], col: Dict[str,Any]) -> Dict[str,Any]:
    ...
    if mod == "Cohort Retention (CSV)":
        ...
        k = _cohort(df)
        out = {"status":"done","value":k["value"],"summary":_with_filter_prefix(row, k["summary"]), "last_run": now_ts()}
    ...
    if mod == "Pricing Power (CSV)":
        df = materialize_df(row["source"])
        k = _pricing(df)
        return {"status":"done","value":k["value"],"summary":_with_filter_prefix(row, k["summary"]), "last_run": now_ts(), **k}
...
def _with_filter_prefix(row: Dict[str,Any], summary: str) -> str:
    if row["row_type"] != "table": return summary
    filt = SS["row_filters"].get(row["source"], {})
    col, vals = filt.get("column"), filt.get("values", [])
    if col and vals:
        return f"[Segment: {col} ∈ {list(vals)}] {summary}"
    return summary
...
def export_results_csv(approved_only: bool = False) -> bytes:
    ...
    for (rid,cid), res in SS["results"].items():
        if approved_only and not SS["approvals"].get((rid,cid), False):
            continue
        ...
        out.append(dict(
            ...
            note=SS["notes"].get((rid,cid), "")
        ))
    return pd.DataFrame(out).to_csv(index=False).encode("utf-8")

def export_results_pdf(approved_only: bool = False) -> bytes:
    if not REPORTLAB_OK: raise RuntimeError("reportlab not installed")
    buf = io.BytesIO()
    c = canvas.Canvas(buf, pagesize=LETTER)
    w,h = LETTER
    c.setFont("Helvetica-Bold", 12)
    title = "TransformAI — QoE Summary (Approved only)" if approved_only else "TransformAI — QoE Summary (All results)"
...
def plot_retention_heatmap(curve: List[float]):
    ...
    if not curve:
        ...
        return
    z = np.tile(curve, (5,1))  # simple synthetic heatmap from the curve
    if PLOTLY_OK:
...
with tab_data:
    ...
    with st.expander("Map CSV Schema (click to edit) + Optional Segment Filters", expanded=True if SS["csv_files"] else False):
        for name, df in SS["csv_files"].items():
            ...
            # NEW: simple segment filter UI per CSV
            st.caption("Optional segment filter for this CSV (limits analyses to chosen slice)")
            fcol = st.selectbox("Filter column", ["— None —"] + list(df.columns), key=f"segcol:{name}")
            if fcol == "— None —":
                SS["row_filters"][name] = {}
            else:
                # unique values (cap to 100 for UI)
                try:
                    uniq = pd.Series(df[fcol].astype(str).unique()).dropna().tolist()
                except Exception:
                    uniq = []
                if len(uniq) > 100:
                    uniq = uniq[:100]
                fvals = st.multiselect("Values", uniq, key=f"segvals:{name}")
                SS["row_filters"][name] = {"column": fcol, "values": fvals}
...
with tab_run:
    ...
    # NEW: Run All Mapped
    with st.expander("Run utilities (Row / Column / All / Cache)", expanded=True):
        rows = SS["rows"]; cols = SS["columns"]
        by_mod = {c["module"]: c["id"] for c in cols}

        # Respect cache toggle
        respect_cache = st.checkbox("Respect cache", value=not SS.get("force_rerun", False), key="respect_cache_all")

        # Run by Row
        c1, c2, c3 = st.columns([2,2,1])
        with c1:
            sel_row = st.selectbox("Row to run (all mapped modules)", [(r["id"], r["alias"]) for r in rows], format_func=lambda t: t[1]) if rows else None
        with c3:
            if sel_row and st.button("Run row"):
                rid = sel_row[0]
                sel_mods = SS["matrix"].get(rid, set())
                pairs = []
                for m in sel_mods:
                    cid = by_mod.get(m)
                    if cid: pairs.append((rid, cid))
                enqueue_pairs(pairs, respect_cache=respect_cache)
                run_queued_jobs()
                st.success(f"Row ran: {len(pairs)} cell(s).")

        # Run by Column
        c4, c5 = st.columns([2,1])
        with c4:
            sel_col = st.selectbox("Column to run (for all mapped rows)", [(c["id"], f"{c['label']} · {c['module']}") for c in cols], format_func=lambda t: t[1]) if cols else None
        with c5:
            if sel_col and st.button("Run column"):
                cid = sel_col[0]
                cdef = next((c for c in cols if c["id"]==cid), None)
                pairs = []
                if cdef:
                    for r in rows:
                        if cdef["module"] in SS["matrix"].get(r["id"], set()):
                            pairs.append((r["id"], cid))
                enqueue_pairs(pairs, respect_cache=respect_cache)
                run_queued_jobs()
                st.success(f"Column ran: {len(pairs)} cell(s).")

        st.divider()
        if st.button("Run ALL mapped"):
            pairs = []
            for r in SS["rows"]:
                rid = r["id"]
                for m in SS["matrix"].get(rid, set()):
                    cid = by_mod.get(m)
                    if cid: pairs.append((rid, cid))
            enqueue_pairs(pairs, respect_cache=respect_cache)
            run_queued_jobs()
            st.success(f"All mapped ran: {len(pairs)} cell(s).")

        st.divider()
        if st.button("Invalidate all caches (force fresh next time)"):
            SS["cache_key"].clear()
            st.success("Cache keys cleared. Next runs will compute fresh if 'Respect cache' is off.")
    ...
    if SS["results"]:
        ...
        with c1:
            st.download_button("Export results CSV (all)", data=export_results_csv(False), file_name="transformai_results.csv")
        with c2:
            st.download_button("Export results CSV (approved only)", data=export_results_csv(True), file_name="transformai_results_approved.csv")
        with c3:
            if REPORTLAB_OK:
                st.download_button("Export MEMO (approved only)", data=export_results_pdf(True), file_name="TransformAI_Memo_approved.pdf")
...
with tab_sheet:
    ...
    # Quick focus/run
    if SS["rows"] and qoe_cols:
        rpick = st.selectbox("Pick a row", [(r["id"], r["alias"]) for r in SS["rows"]], format_func=lambda t: t[1], key="sheet_rpick")
        cpick = st.selectbox("Pick a column", [(c["id"], f"{c['label']} · {c['module']}") for c in qoe_cols], format_func=lambda t: t[1], key="sheet_cpick")
        cc1, cc2 = st.columns(2)
        with cc1:
            if st.button("Run selected pair"):
                enqueue_pairs([(rpick[0], cpick[0])], respect_cache=True)
                run_queued_jobs()
                st.success("Cell executed.")
        with cc2:
            if st.button("Focus in Review"):
                SS["pending_focus"] = (rpick[0], cpick[0])
                SS["last_focus"] = SS["pending_focus"]
                st.success("Selected. Switch to the Review tab — it will be pre-filled.")
...
with tab_review:
    ...
    # Prefer last focus (after runs) or pending focus (from Sheet)
    default_rid, default_cid = None, None
    if SS.get("pending_focus"):
        default_rid, default_cid = SS["pending_focus"]
        SS["pending_focus"] = None
    elif SS.get("last_focus"):
        default_rid, default_cid = SS["last_focus"]
    ...
    def_idx_col = 0
    if default_rid:
        for i,(rid,label) in enumerate(row_opt):
            if rid == default_rid:
                def_idx_row = i; break
    if default_cid:
        for i,(cid,label) in enumerate(col_opt):
            if cid == default_cid:
                def_idx_col = i; break

    csel1, csel2, csel3 = st.columns([2,2,1])
    with csel1:
        rid = st.selectbox("Row", row_opt, index=def_idx_row if row_opt else 0, format_func=lambda t: t[1]) if row_opt else None
    with csel2:
        cid = st.selectbox("Column", col_opt, index=def_idx_col if col_opt else 0, format_func=lambda t: t[1]) if col_opt else None
    with csel3:
    ...
    if not (rid and cid):
        ...
        with run_cols[0]:
            if st.button("Run this cell now", type="primary"):
                enqueue_pairs([(rid, cid)], respect_cache=not SS.get("force_rerun", False))
                run_queued_jobs()
                res = SS["results"].get((rid, cid))

        # Approvals + Notes
        app_cols = st.columns([1,2,5])
        with app_cols[0]:
            current = SS["approvals"].get((rid,cid), False)
            if st.button("Mark Approved ✅" if not current else "Mark Needs Review ⛔"):
                SS["approvals"][(rid,cid)] = not current
                st.toast("Status updated")
        with app_cols[1]:
            note_key = f"note:{rid}:{cid}"
            note_val = SS["notes"].get((rid,cid), "")
            new_note = st.text_input("Note", value=note_val, key=note_key, placeholder="Optional reviewer note…")
            if new_note != note_val:
                SS["notes"][(rid,cid)] = new_note

        if not res:
            st.warning("No result yet. Click **Run this cell now**.")
        else:
            filt = SS["row_filters"].get(r["source"], {}) if r["row_type"]=="table" else {}
            seg = f" · Segment: {filt.get('column')} ∈ {filt.get('values')}" if filt.get("column") and filt.get("values") else ""
            st.caption(f"**{r['alias']}** → **{c['label']}** ({c['module']}){seg}")
...
with tab_memo:
    ...
    st.caption("Approved cells are prioritized in exports (see Run tab).")
    if REPORTLAB_OK:
        st.write("Use **Run → Export MEMO (approved only)** to preview.")
    else:
