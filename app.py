...
ai_mode = st.sidebar.toggle("Enable AI-generated plays", value=False,
...
sample = st.selectbox("Choose a sample company",
...
if st.button("Generate Playbook", type="primary"):
    ...
    if any([survey_file, market_file, macro_file, stockouts_file, util_file]) or use_sample_ev:
        ev = load_sample_evidence(use_sample_ev=use_sample_ev,
    ...
    with st.spinner("Analyzing…"):
        if ai_mode:
            ...
            import requests
            r = requests.post(f"{backend_url.rstrip('/')}/analyze",
...
if "last_result" in st.session_state:
    ...
    try:
        if uploaded is not None:
            ...
            df_plot = pd.read_csv("data/sample_healthco_long.csv") if "HealthCo" in sample and "8q" in sample                       else pd.read_csv("data/sample_retailco_long.csv") if "RetailCo" in sample and "8q" in sample                       else None
    ...
    chips = chips_from_signals(signals)
    if chips:
        st.write("  ".join(chips))
    ...
    type_colors = {"pricing":"#10B981","retention":"#3B82F6","supply":"#F59E0B","utilization":"#A855F7","claims":"#EC4899","referrals":"#22D3EE"}
    ...
    for play in data["plays"]:
        with st.container(border=True):
            ...
            st.markdown(f"<div style='height:6px;background:{bar_color};margin:-0.5rem -0.5rem 0.75rem -0.5rem;border-radius:8px 8px 0 0'></div>", unsafe_allow_html=True)
            ...
            with left:
                ...
                if play.get("type") == "pricing":
                    pct = st.slider("Assumed price increase", 0.0, 0.12, float(defaults.get("pricing_uplift", 0.06)), 0.01, key=f"slider_{play['id']}")
                    gm = st.slider("Gross margin", 0.1, 0.7, float(kpis.get('gross_margin', 0.35)), 0.01, key=f"gm_{play['id']}")
                    adoption = st.slider("Adoption", 0.1, 1.0, float(defaults.get("pricing_adoption", 0.6)), 0.05, key=f"adopt_{play['id']}")
                    uplift = kpis["revenue"] * pct * gm * adoption
                elif play.get("type") == "retention":
                    reduce_pp = st.slider("Churn reduction (pp)", 0.00, 0.10, float(defaults.get("retention_reduce_pp", 0.03)), 0.005, key=f"slider_{play['id']}")
                    gm = st.slider("Gross margin", 0.1, 0.7, float(kpis.get('gross_margin', 0.35)), 0.01, key=f"gm_{play['id']}")
                    uplift = kpis["revenue"] * reduce_pp * gm * 0.5
                elif play.get("type") in ["supply", "utilization"]:
                    delta = st.slider("Impact factor", 0.00, 0.05, float(defaults.get("ops_delta", 0.02)), 0.005, key=f"slider_{play['id']}")
                    gm = st.slider("Gross margin", 0.1, 0.7, float(kpis.get('gross_margin', 0.35)), 0.01, key=f"gm_{play['id']}")
