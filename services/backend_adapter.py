...
class MockBackend(BaseBackend):
    ...
    def _sleep(self): time.sleep(self.latency_ms/1000.0)

    def health(self): self._sleep(); return {"ok": True, "flags": {"FF_GRID_RUNTIME": True, "FF_MEMO": True}, "mode": "mock"}
    ...
    def list_cells(self, grid_id): self._sleep(); return list(self._cells.get(grid_id, []))
    ...
    def approve_cell(self, cell_id: str, note: str = ""):
        self._sleep(); self._approved.add(cell_id); return {"ok": True, "cell_id": cell_id, "note": note}
    ...
    def export_pdf(self, project_id: str, template: str = "ic_default_v1"):
        self._sleep(); return {"ok": True, "url": f"mock://export/{project_id}/{template}.pdf"}

# -------- HTTP (future: talks to real FastAPI if you paste a URL) --------
@dataclass
class HttpBackend(BaseBackend):
    ...
    def _get(self, path: str, **kw): return requests.get(self.base_url+path, headers=self._headers(), timeout=self.timeout_s, **kw)
    def _post(self, path: str, json=None, **kw): return requests.post(self.base_url+path, json=json, headers=self._headers(), timeout=self.timeout_s, **kw)

    def health(self): r=self._get("/health"); r.raise_for_status(); out=r.json(); out["mode"]="http"; return out
    def create_grid(self, project_id, name, columns, rows):
        r=self._post("/grid", json={"project_id":project_id,"name":name,"columns":columns,"rows":rows}); r.raise_for_status(); return r.json()
    def list_cells(self, grid_id): r=self._get("/cells", params={"grid_id":grid_id}); r.raise_for_status(); return r.json()
    def run_cells(self, grid_id):
        r=self._post(f"/grid/{grid_id}/run", json={})
        if r.status_code in (404,405): return {"ok": True, "updated": 0, "note": "run endpoint not implemented"}
        r.raise_for_status(); return r.json()
    def approve_cell(self, cell_id: str, note: str = ""):
        r=self._post(f"/cells/{cell_id}/approve", json={"note": note})
        if r.status_code in (404,405): return {"ok": True, "cell_id": cell_id, "note": note, "simulated": True}
        r.raise_for_status(); return r.json()
    def memo(self, project_id: str):
        r=self._get("/memo", params={"project_id": project_id})
        if r.status_code in (404,405): return {"sections":[{"title":"Memo","content":"Backend memo not implemented yet."}]}
        r.raise_for_status(); return r.json()
    def export_pdf(self, project_id: str, template: str = "ic_default_v1"):
        r=self._post("/export", json={"project_id":project_id,"template":template,"include_appendix":True})
        if r.status_code in (404,405): return {"ok": True, "url": "mock://export.pdf", "simulated": True}
        r.raise_for_status(); return r.json()
